...
class FPDF
  ...
  def method_missing(name, *args)
    new_name = name.to_s.camelize
    if self.respond_to? new_name
      self.class.class_eval "alias #{name} #{new_name}"
      self.send name, *args
    else
      raise "Method not found in FPDF: #{name}(#{args.join ', '}) (would have called #{new_name})"
    end
  end
end
